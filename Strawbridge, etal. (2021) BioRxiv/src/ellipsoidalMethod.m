function idx = ellipsoidalMethod(points)
    % Used by MINS   
    % Lou, et al., (2014) Stem Cell Reports
    
    threshold = 0.95;

    ellipsoid = fitEllipsoid(points);

    d = calculateDistance(points, ellipsoid);    
    
    idx = d > threshold;       

end

function ellipsoid = fitEllipsoid(points)

     [center, radii] = ellipsoid_fit_new( points);

    ellipsoid.c = radii;
    ellipsoid.origin = center;
    
end

function d = calculateDistance(points, ellipsoid) 

    d = zeros(size(points,1),1);

    f = @(p,c,Ori)  ((p(1)-Ori(1))./c(1)).^2 ...
                  + ((p(2)-Ori(2))./c(2)).^2 ...
                  + ((p(3)-Ori(3))./c(3)).^2;

    for i = 1:size(points,1)
        
        d(i) = f(points(i,:),ellipsoid.c,ellipsoid.origin);
