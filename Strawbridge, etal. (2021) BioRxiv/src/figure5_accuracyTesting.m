function figure5_accuracyTesting(method,figNum)
    ...
    n_noise_levels = 100;
    noise_levels = linspace(0,0.25,n_noise_levels);
    r_sphere = 1;
    n_sphere_points = 100;
    O_sphere = [0 0 0];
    ...
    sensitivity = zeros(n_ball_radii,n_noise_levels,n_spheres);
    ...
    for i = 1:n_ball_radii
        ...
        for j = 1:n_noise_levels
            
            noise = noise_levels(j);
            
            parfor k = 1:n_spheres

                % Make Ball
                [B] = makeBallRandom(r,n_ball_points, O_ball);

                % Make sphere
                [S] = makeSphereRandom(r_sphere,n_sphere_points, O_sphere,noise);    
                ...
                sensitivity(i,j,k) = stats(1);
                specificity(i,j,k) = stats(2);
    ...
    [r_ball_mat,noise_mat] = meshgrid(r_ball,noise_levels);
     
    stats = struct('r_ball',r_ball_mat,...
                   'noise',noise_mat,....
